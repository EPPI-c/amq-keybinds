...
(function() {
    ...
    var _ = function(input, o) {
        ...
        configure(this, {
            ...
            maxItems: 10,
            autoFirst: false,
            submitKeys: _.ENTER_KEYS,
            nextKeys: _.DOWN_KEYS,
            previousKeys: _.UP_KEYS,
            cancelKeys: _.ESCAPE_KEYS,
            data: _.DATA,
        ...
        this._events = {
            input: {
                ...
                "keydown": function(evt) {
                    // console.log(me.submitKeys)
                    // console.log(evt)
                    // If the dropdown `ul` is in view, then act on keydown for the configured keys
                    // Default (Enter / Esc / Up / Down)
                    if (me.opened) {
                        if ($.in(evt, me.submitKeys) && me.selected) {
                            evt.preventDefault();
                            me.select(undefined, undefined, evt);
                        }
                        else if ($.in(evt, me.cancelKeys)) {
                            evt.preventDefault();
                            me.close({ reason: "esc" });
                        }
                        else if ($.in(evt, me.nextKeys)) {
                            evt.preventDefault();
                            me["next"]();
                        }
                        else if ($.in(evt, me.previousKeys)) {
                            evt.preventDefault();
                            me["previous"]();
                        }
                    }
        ...
        if (this.input.hasAttribute("data-submitkeys")) {
            var submitkeys = this.input.getAttribute("data-submitkeys")
            this.submitKeys = JSON.parse(submitkeys);
        }
        if (this.input.hasAttribute("data-nextkeys")) {
            var nextkeys = this.input.getAttribute("data-nextkeys")
            this.nextKeys = JSON.parse(nextkeys);
        }
        if (this.input.hasAttribute("data-previouskeys")) {
            var previouskeys = this.input.getAttribute("data-previouskeys")
            this.previousKeys = JSON.parse(previouskeys);
        }
        if (this.input.hasAttribute("data-cancelkeys")) {
            var cancelkeys = this.input.getAttribute("data-cancelkeys")
            this.cancelKeys = JSON.parse(cancelkeys);
        }
    ...
    _.ENTER_KEYS.push(ctrl_y)
    _.DOWN_KEYS.push(ctrl_j)
    _.UP_KEYS.push(ctrl_k)
